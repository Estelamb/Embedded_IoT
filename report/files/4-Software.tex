...
\todo{chequear en todo el docuemnto si zephyr es os o rtos}
...
Additionally, the DeviceTree overlay file (\texttt{nucleo\_wl55jc.overlay}) extends the hardware description of the Nucleo-WL55JC board by defining an \gls{RGB} \gls{LED} structure implemented through \gls{GPIO}-controlled \gls{LED} nodes. Corresponding aliases are included to simplify application-level access to these components. The overlay also configures the \gls{USART}1 peripheral with its associated pin assignments and baud rate, enabling serial communication capabilities required by the console or other \gls{UART}-based interfaces.
