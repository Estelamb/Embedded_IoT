...
The document outlines the hardware elements provided to each student group, including the STM32WL55JC microcontroller platform and the environmental and inertial sensors required for data acquisition. It also describes the software tools, \gls{RTOS} configuration, and development workflow that must be used throughout the project. Furthermore, it details the functional requirements, sensing capabilities, operational modes, and performance constraints that the final \gls{IoT} system must fulfill.

Finally, additional sections define the expected project tasks, development methodology, evaluation criteria, and the tentative schedule for completion of the assignment. These specifications serve as the baseline for ensuring design consistency, functional correctness, and reproducibility across all student projects.
...
This project integrates both hardware and software development activities. Students must interface several digital and analog sensors, configure low-level peripherals (\gls{I2C}, \gls{UART}, \gls{ADC}, \gls{GPIO}, \gls{PWM}), and implement multitasking using Zephyr \gls{OS}. The final embedded application must operate in distinct modes, manage periodic measurements, compute statistical parameters, handle event-based notifications, and provide visual feedback via a \gls{RGB} \gls{LED}.
...
The project began with an in-depth study of the provided technical specifications and sensor documentation. All functional, hardware, and timing requirements were reviewed to establish a clear design baseline. This included understanding the sensing ranges, communication interfaces, and operating constraints imposed by the STM32WL55JC microcontroller and Zephyr OS.
...
\begin{itemize}
    ...
    \item Setup of the MMA8451Q accelerometer for multi-axis inertial measurements.
    \item Interfacing and configuring the FGPMMOPA6H \gls{GPS} module via \gls{UART}.
    \item Implementation of an \gls{RGB} \gls{LED} driver using \gls{PWM} emulation for status indication.
\end{itemize}

A breadboard-based prototype was assembled to confirm electrical correctness, noise stability, and peripheral compatibility.

\subsubsection{Software Development}

Software implementation was carried out using Zephyr \gls{OS}, structured with a multitasking architecture. Key activities included:
...
\begin{itemize}
    ...
    \item Clean, documented source code developed under Zephyr \gls{OS}.
